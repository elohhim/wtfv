...
const getFixVersionAndPlus = (yearLittle: string, week: number, day: number): [string, string] => {
  if (day === 4) {
    return [
        `${yearLittle}.${week + 1} or ${yearLittle}.${week + 2}`,
        `1 or 2`
    ];
  } else {
...
const App = () => {
  ...
  const thursdayExplain = '(Thursdays are tricky, there is release to staging on Thursday, check if the release was done already)';
  return (
      <div className="App">
        <header>
        ...
        <main>
          ...
          <details>
            ...
            <p>
              The year is {yearBig}<mark>{yearLittle}</mark>
              and it is <mark>{ordinal(week)} week</mark>
              of the year (according to {isoLink}).
              It is <mark>{dayName}</mark> {day === 4 ? thursdayExplain : ''} so we need to add <mark>{plus}</mark> to
              the week number.
        ...
        <footer>
          <ul>
            <li>TBH I don't know if fix version uses {isoLink} week numbering, it might be pure coincidence that it
              works for 2020.
            </li>
            <li>
